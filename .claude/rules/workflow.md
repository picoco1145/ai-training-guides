...
2. 이미지 제작 완료 후 **반드시 image-reviewer 검수** — Pass 없이 콘텐츠 검수 진행 금지
3. 콘텐츠 완성 후 **반드시 reviewer 검수** — Pass 없이 Notion 배포 금지
4. 가이드 생성/수정 시 **반드시 registry 업데이트**
...
- **이미지 완료 직후 반드시 → image-reviewer (이미지 품질 검수)** — Pass 없이 다음 단계 금지
- image-reviewer Pass 후 → reviewer (초보자 관점 콘텐츠 검수)
- Notion 배포 → publisher
- 에러 발생 → debugger로 분석
...
- 에이전트별 출력 디렉토리 준수:
  ...
  - image-reviewer: outputs/reviews/{번호}_image_review.md
